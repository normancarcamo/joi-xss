...
describe("joi-xss", () => {
  ...
  it("expect to escape html chars in an input of string type", () => {
    ...
    // When:
    let { value } = Joi.string().xss().validate(input);
  ...
  it("expect to use options passed through", () => {
    ...
    // When:
    let result = Joi.object().unknown(true).xss({ stripIgnoreTag: true }).validate(input);
  ...
  it("expect escape deeply in objects", () => {
    ...
    for (i; i < inputs.length; i++) {
      ...
      // When:
      let result = Joi.object().unknown(true).xss(options).validate(inputs[i]);
  ...
  it("expect escape deeply in arrays", () => {
    ...
    for (i; i < inputs.length; i++) {
      ...
      // When:
      let result = Joi.array().xss(options).validate(inputs[i]);
